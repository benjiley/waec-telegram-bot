...
def main_menu(chat_id):
    ...
    markup.row("📝 Register", "📚 Take Quiz")
    markup.row("🎓 Take Lecture", "📄 My Info")
...
def register(message):
    ...
    bot.send_message(chat_id, "What is your full name?")
...
# Lecture menu
@bot.message_handler(func=lambda m: m.text == "🎓 Take Lecture")
def send_lecture_menu(message):
    chat_id = message.chat.id
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    for subject in lectures:
        markup.add(subject)
    bot.send_message(chat_id, "Choose a subject:", reply_markup=markup)

# Lecture content
@bot.message_handler(func=lambda m: m.text in lectures.keys())
def lecture_content(message):
    chat_id = message.chat.id
    subject = message.text
    bot.send_message(chat_id, lectures[subject], parse_mode="Markdown")
    main_menu(chat_id)
...
def my_info(message):
    ...
    if data and "name" in data:
        ...
        bot.send_message(chat_id, f"👤 Name: {name}\n🏫 Class: {class_}\n📚 Subjects: {subjects}")
    else:
        bot.send_message(chat_id, "You're not registered yet. Tap 📝 Register.")
